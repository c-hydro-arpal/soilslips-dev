...
class DriverAnalysis:
    ...
    def __init__(self, time_run, time_range, ancillary_dict, dst_dict,
                 ...
                 flag_domain_data='alert_area', flag_time_data='time',
        ...
        self.flag_domain_data = flag_domain_data
    ...
    def dump_scenarios(self, scenarios_collections):
        ...
        scenarios_file_list, scenario_key_list, scenario_flag_merge = filter_file_obj(scenarios_file_path)
        ...
        if not scenario_flag_merge:
            for group_data_key in self.structure_data_group.keys():
                ...
                logging.info(' -----> Save Alert Area ' + group_data_key + '  ... ')
                ...
                if not os.path.exists(file_path):
                    ...
                    if file_data is not None:
                        ...
                        logging.info(' -----> Save Alert Area ' + group_data_key + '  ... DONE')
                    else:
                        logging.info(' -----> Save Alert Area ' + group_data_key + '  ... SKIPPED. Datasets are undefined.')

                else:
                    logging.info(' -----> Save Alert Area ' + group_data_key + '  ... SKIPPED. Datasets previously saved.')
            ...
            logging.info(' -----> Save Alert Area collections ... ')
            if not os.path.exists(file_path):
                ...
                if merged_data is not None:
                    ...
                    logging.info(' -----> Save Alert Area collections ... DONE')

                else:
                    logging.info(' -----> Save Alert Area collections ... SKIPPED. Datasets are undefined')

            else:
                logging.info(' -----> Save Alert Area collections  ... SKIPPED. Datasets previously saved.')
            ...
            if not os.path.exists(file_path_scenarios_selection):
                if file_path_indicators_selection is not None:
                    ...
                    for file_path_step in file_path_indicators_selection:
                        ...
                        file_domain_data = file_obj_step[self.flag_domain_data]
                        ...
                        if 'event_domain' not in list(file_event_step.keys()):
                            file_event_step['event_domain'] = file_domain_data
