            ...
            if not (os.path.exists(file_path_ancillary_map or os.path.exists(file_path_ancillary_point))):
                ...
                if file_dframe is not None:
                    ...
                    if check_idxs_var_min.__len__() > 0:
                        logging.warning(' -------> Some values are less then the "var_min" limit ... ')
                        map_out_2d[map_out_2d < var_min] = var_min
                        logging.warning(
                            ' -------> Some values are less then the "var_min" limit ... FIX MAP VALUES TO MIN LIMIT')
                    ...
                    if check_idxs_var_max.__len__() > 0:
                        ...
                        logging.warning(
                            ' -------> Some values are less then the "var_max" limit ... FIX MAP VALUES TO MAX LIMIT')
