...
from lib_data_io_fx_kernel import convert_dframe2array, \
    filter_dataframe_columns, add_dataframe_columns, drop_dataframe_columns, order_dataframe_columns
...
def organize_fx_kernel_datasets_out(fx_output, fx_datasets=None,
                                    ...
                                    fx_name_soil_slips_output='soil_slips_prediction'):
    if fx_datasets is not None:
        ...
        fx_dframe[fx_name_soil_slips_output] = fx_output
    else:
...
def organize_fx_kernel_parameters(
        fx_attrs,
        fx_name_kernel_matrix='kernel_matrix', fx_name_kernel_coefficient='kernel_coefficient',
        fx_name_kernel_type='kernel_type', fx_name_kernel_exponent='kernel_exponent'):

    if 'fx_training_matrix' in list(fx_attrs.keys()):
        fx_training_matrix = fx_attrs['fx_training_matrix']
    else:
        log_stream.error(' ===> Attribute "fx_training_matrix" must be in the attributes obj')
        raise RuntimeError('Attribute is mandatory to apply the method')
    ...
    fx_obj = {fx_name_kernel_matrix: fx_training_matrix, fx_name_kernel_coefficient: fx_training_coefficient,
...
def exec_fx_kernel(kernel_datasets,
                   kernel_matrix=None, kernel_coefficient=None,
    ...
    kernel_predictors = regularizedKernLSTest(
        kernel_coefficient, kernel_matrix, kernel_type, kernel_exponent,
        kernel_datasets_norm)
